...
| `ENABLE_JSON_MCP` | нет | `true` включает экспериментальный `/mcp-json`; по умолчанию `false` |
...
## Экспериментальный JSON MCP

При `ENABLE_JSON_MCP=true` появляется отдельный Streamable HTTP endpoint `/mcp-json` с тем же Bearer-ключом, Origin-политикой и общим лимитом запросов. В окне «Подключить ИИ» выберите «JSON MCP — экспериментальный», чтобы получить конфигурацию `bpmn_json` и рекомендуемый skill. При выключенной настройке endpoint возвращает `404`; существующий `/mcp` продолжает работать.

JSON является форматом обмена, не новым хранилищем: веб-редактор, REST и volume остаются XML. ИИ может читать фрагменты без геометрии и отправлять короткие атомарные пакеты операций. Старые диаграммы при включении endpoint не переписываются.

Начинайте проверку на копиях. Существующий ручной layout сохраняется по умолчанию; автоматическая перестройка требует явного режима. Неподдерживаемые или потенциально теряющие данные преобразования отклоняются.

Контракт, примеры, ограничения и поведение при конфликтах описаны в [JSON MCP](docs/json-mcp.md). Готовый instruction-only skill находится в [SKILL.md](skills/bpmn-json-modeler/SKILL.md).
